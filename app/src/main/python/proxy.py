...
BASE_DIR = Path(os.environ.get("DS_DATA_DIR") or Path(__file__).parent)
...
def _load_config_sync() -> dict:
    ...
    if not accounts:
        return {}
    first = accounts[0]
    # get_all_accounts() 只返回脱敏摘要（token_masked），
    # 模型发现等内部调用需要真实 token/headers，从账号对象补齐。
    # 注：Android 垫片环境 httpx 拒绝空 Bearer 头（curl_cffi 允许），
    # 因此这里必须带上真实 token，否则模型发现失败。
    ...
    return {
        ...
        "accounts": accounts,
        "token": getattr(acc, "token", "") if acc else "",
        "headers": getattr(acc, "headers", {}) if acc else {},
    }
...
@app.post("/api/accounts/import-token")
async def import_token(data: dict, creds: HTTPBasicCredentials = Depends(verify_admin)):
    """从内置浏览器登录态导入账号（Android 客户端使用）。

    客户端在 WebView 中登录 chat.deepseek.com 后读取 localStorage 里的
    userToken 并调用本接口；此处先向上游校验 token 有效性再落库。
    """
    token = (data.get("token") or "").strip()
    if not token:
        raise HTTPException(400, "缺少 token")

    ua = (data.get("user_agent") or "").strip() or (
    ...
    # 上游校验：拿到 user id 说明 token 有效
    try:
        resp = cffi_requests.get(
            "https://chat.deepseek.com/api/v0/users/current",
            headers=headers, timeout=20, impersonate="chrome120",
            proxies=_get_proxy_dict(),
        )
        payload = resp.json()
    except Exception as e:
        raise HTTPException(502, f"校验 token 失败: {e}")

    biz = (payload.get("data") or {}).get("biz_data") or {}
    if resp.status_code != 200 or not biz:
        msg = (payload.get("data") or {}).get("biz_msg") or payload.get("msg") or "token 无效"
        raise HTTPException(401, f"token 校验未通过: {msg}")

    mobile = biz.get("mobile_number") or ""
    email = biz.get("email") or ""
    label = email or mobile or biz.get("id") or "browser-login"

    # 创建新会话，拿到 session_id 才能发起对话
    session_id = ""
    try:
        sresp = cffi_requests.post(
            "https://chat.deepseek.com/api/v0/chat_session/create",
            json={}, headers=headers, timeout=15, impersonate="chrome120",
            proxies=_get_proxy_dict(),
        )
        if sresp.status_code == 200:
            sbiz = (sresp.json().get("data") or {}).get("biz_data") or {}
            session_id = sbiz.get("chat_session", {}).get("id") or sbiz.get("id") or ""
            print(f"[import-token] session_id={session_id}")
    except Exception as e:
        print(f"[import-token] create session failed: {e}")

    existing = config_manager.get_account_by_label(label)
    if existing:
        existing.token = token
        existing.headers = headers
        existing.session_id = session_id
        existing.is_valid = True
        existing.login_time = time.strftime("%Y-%m-%d %H:%M:%S")
        config_manager.save()
        return {"ok": True, "account_label": label, "updated": True}

    acc = DsAccount(
        account_label=label,
        login_type="email" if email else "phone",
        _email=email,
        _mobile=mobile,
        token=token,
        session_id=session_id,
    ...
    config_manager.add_account(acc)
    return {"ok": True, "account_label": label, "added": True}
