# file-and-vision-handling Specification

## Purpose
Define how the proxy accepts file and image inputs, uploads them to DeepSeek, prepares them for chat or Responses use, and forwards ready file references to upstream requests.
## Requirements
### Requirement: Text file ingestion for chat and Responses requests
The proxy SHALL accept OpenAI-style file content parts and Responses-style inline file input items containing inline file data, then forward successfully parsed files to DeepSeek as referenced files.
...
The proxy SHALL accept OpenAI-style and Responses-style inline or URL-based image inputs for vision models and prepare them for DeepSeek vision access.
